...
namespace e
{
	...
	uint8* Encoder0::WriteMarkers(uint8* out, uint32 format, uint32 width, uint32 height)
	{
	...
	void Encoder0::ReadYUV422(JpegEncoderStruct* jes, uint8* input)
	{
	...
	void Encoder0::RGB2YUV422(uint8* output, uint8* input, int width, int height)
	{
		...
		for (uint32 i = size; i > 0; i--)
		{
			...
			Y0 = clamp0255((77 * R + 150 * G + 29 * B) >> 8);
			Y1 = clamp0255((77 * R1 + 150 * G1 + 29 * B1) >> 8);
	...
	void Encoder0::Initialize(JpegEncoderStruct* jes, uint32 format, uint32 width, uint32 height)
	{
		...
		fnReadFormat = &Encoder0::ReadYUV422;
	}

	void Encoder0::InitializeQuantizationTables(uint32 qualityFactor)
	{
		...
		for (int i = 0; i < 64; i++)
		{
			...
			ILQT[index] = DSP_Division(0x8000, value);

			value = chrominance_quant_table[i] * qualityFactor;
			value = (value + 0x200) >> 10;
	...
	uint8* Encoder0::CloseBitStream(uint8* output)
	{
		if (bitindex > 0)
		{
			lcode <<= (32 - bitindex);
	...
	void Encoder0::Quantization(int16* const data, uint16* const quant_table)
	{
		int32 value;
		for (int i = 63; i > 0; i--)
		{
	...
	uint8* Encoder0::Haffman(JpegEncoderStruct* jes, uint16 component, uint8* output)
	{
	...
	uint8* Encoder0::EncodeMCU(JpegEncoderStruct* jes, uint32 format, uint8* output)
	{
		DCT(Y1);
	...
	uint32 Encoder0::EncodeImage(uint8* output, uint8* input, uint32 qualityFactor, uint32 format, uint32 width, uint32 height)
	{
		uint8* out = output;
