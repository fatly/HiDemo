...
public:
	...
	virtual void RemoveReceiver(ReceiverImplBase<ReturnT, ParamT>* receiver)
	{
		...
		ReceiversVector::iterator it = receivers.begin();
	...
	{
		ReceiversVector::reverse_iterator it = receivers.rbegin();
		for (; it != receivers.rend(); ++it)
		{
			(*it)->Receive(param);
		}
	...
	public:
		...
		{
			if (index >= _tbl.receivers.size())
			{
				return NULL;
			}

			for ( ; index < _tbl.receivers.size(); )
			{
				ptr = _tbl.receivers[ index++ ];
				if (ptr)
				{
					return ptr;
				}
...
public:
	...
	{
		ObserversVector::iterator it = observers.begin();
